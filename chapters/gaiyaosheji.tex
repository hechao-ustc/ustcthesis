...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{数据源创建时序图.png}
  \caption{数据源创建时序图}
  \label{fig:数据源创建时序图}
\end{figure}
...
其中表的创建需要和安全中心、hive metastore、mysql进行交互，一个createTable的request建立后，
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{DLA创建Iceberg表时序图.png}
